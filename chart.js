...
(function() {
    ...
    e.defaults = {
        global: {
            ...
            tooltipXOffset: 10,
            tooltipTemplate: "<%if (label){%><%=label%> <%}%><%if(label!='No Data Found'){%>: <%=value%><%}%>",
                ...
                if (this.activeElements = t, this.draw(), this.options.customTooltips && this.options.customTooltips(!1), t.length > 0) if (this.datasets && this.datasets.length > 1) {
                    ...
                    d = function() {
                        ...
                        function(t) {
                            l.push(t.x),
                            r.push(t.y),
                            c.push(s.template(t.datasetLabel + " : " + this.options.multiTooltipTemplate, t)),
                            u.push({
...
function() {
    ...
    s = {
        ...
        barValueSpacing: 5,
        barDatasetSpacing: 1,
        legendTemplate: '<div style="width:auto;float:right;"><ul class="<%=name.toLowerCase()%>-legend"><% for (var i=0; i<datasets.length; i++){%><li><span style="background-color:<%=datasets[i].fillColor%>;border:2px solid <%=datasets[i].strokeColor%>;"></span><%if(datasets[i].label){%><%=datasets[i].label%><%}%><%if(datasets[i].value){%> - <%if(datasets[i].symbol_char){%><%=datasets[i].symbol_char%><%}%> <%=datasets[i].value%><%}%></li><%}%></ul></div>'
    };
    i.Type.extend({
        name: "Bar",
        defaults: s,
...
function() {
    ...
    s = {
        ...
        animateRotate: !0,
        animateScale: !1,
        legendTemplate: '<div style="width:auto;float:right;"><ul class="<%=name.toLowerCase()%>-legend"><% for (var i=0; i<segments.length; i++){%><li><span style="background-color:<%=segments[i].fillColor%>"></span><%if(segments[i].label){%><%=segments[i].label%><%}%><%if(segments[i].value && segments[i].label!=\'No Data Found\'){%> - <%if(segments[i].symbol_char){%><%=segments[i].symbol_char%><%}%> <%=segments[i].value%><%}else if(segments[i].label!=\'No Data Found\'){%> - <%if(segments[i].symbol_char){%><%=segments[i].symbol_char%><%}%> 0<%}%></li><%}%></ul></div>'
    };
    i.Type.extend({
        name: "Doughnut",
        defaults: s,
...
function() {
    ...
    s = {
        ...
        datasetFill: !0,
        legendTemplate: '<div style="width:auto;float:right;"><ul class="<%=name.toLowerCase()%>-legend"><% for (var i=0; i<datasets.length; i++){%><li><span style="background-color:<%=datasets[i].strokeColor%>"></span><%if(datasets[i].label){%><%=datasets[i].label%><%}%><%if(datasets[i].value){%> - <%if(datasets[i].symbol_char){%><%=datasets[i].symbol_char%><%}%> <%=datasets[i].value%><%}%></li><%}%></ul></div>'
        //legendTemplate: '<ul class="<%=name.toLowerCase()%>-legend"><% for (var i=0; i<datasets.length; i++){%><li><span style="background-color:<%=datasets[i].strokeColor%>"></span><%if(datasets[i].label){%><%=datasets[i].label%><%}%><%if(datasets[i].value){%> - <%=datasets[i].value%><%}%></li><%}%></ul>'
    };
    i.Type.extend({
        name: "Line",
        defaults: s,
